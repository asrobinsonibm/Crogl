...
Now includes a KEV column (1/0) indicating whether any CVE for the advisory appears
...
Optional: specify KEV source (URL or local file: .json/.csv). If omitted, script will try common CISA feed URLs:
...
# We need four buckets; map medium->moderate and unknown->low.
SEV_MAP = {
    "unknown": "low",
    "low": "low",
    "medium": "moderate",
...
def gh_headers(token: str) -> Dict[str, str]:
    h = {
        ...
        "User-Agent": "ghsa-rest-exporter/1.2",
...
def _load_kev_from_json_text(text: str) -> Set[str]:
    """Parse CISA KEV JSON feed. Expected top-level {'vulnerabilities':[{'cveID': 'CVE-...'}, ...]}"""
    try:
        obj = json.loads(text)
    except json.JSONDecodeError:
        return set()
    vulns = obj.get("vulnerabilities") or obj.get("Vulnerabilities") or []
    kev = set()
    for v in vulns:
        # tolerate key variations: cveID, cveId, cve_id, etc.
        cve = v.get("cveID") or v.get("cveId") or v.get("cve_id") or v.get("CVE") or v.get("cve")
        if isinstance(cve, str) and cve.startswith("CVE-"):
            kev.add(cve.strip())
    return kev

def _load_kev_from_csv_text(text: str) -> Set[str]:
    lines = text.splitlines()
    reader = csv.DictReader(lines)
    kev = set()
    # Most feeds use header "cveID"
    for row in reader:
        cve = (row.get("cveID") or row.get("CVE") or row.get("cve") or "").strip()
...
def load_kev_set(kev_source: Optional[str]) -> Set[str]:
    """
    Load the CISA KEV set of CVE IDs from:
     - a provided URL (JSON/CSV),
     - a local file (.json/.csv),
     - or auto-detected default URLs if kev_source is None.
    """
    candidates: List[str] = []
    if kev_source:
        candidates = [kev_source]
    else:
        candidates = DEFAULT_KEV_URLS

    for src in candidates:
        try:
            if _is_url(src):
                print(f"[*] Fetching KEV feed: {src}", file=sys.stderr)
                r = requests.get(src, headers={"User-Agent": "ghsa-rest-exporter/kev"}, timeout=45)
                r.raise_for_status()
                text = r.text
            else:
                p = Path(src)
                if not p.exists():
                    print(f"[kev] Local file not found: {p}", file=sys.stderr)
                    continue
                print(f"[*] Loading KEV from local file: {p}", file=sys.stderr)
                text = _read_local_file(p)

            kev: Set[str] = set()
            src_lower = src.lower()
            if src_lower.endswith(".json") or (text.strip().startswith("{") or text.strip().startswith("[")):
                kev = _load_kev_from_json_text(text)
            else:
                kev = _load_kev_from_csv_text(text)

            if kev:
                print(f"[*] KEV loaded: {len(kev)} CVEs", file=sys.stderr)
                return kev
            else:
                print(f"[kev] Parsed but found 0 CVEs, trying next source…", file=sys.stderr)

        except Exception as e:
            print(f"[kev] Failed to load from {src}: {e}", file=sys.stderr)
            continue

    print("[kev] WARNING: Could not load KEV feed; KEV column will be 0 for all rows.", file=sys.stderr)
    return set()
...
def build_csv_rows_from_advisory(a: Dict[str, Any], kev_set: Set[str]) -> List[Dict[str, Any]]:
    """Return CSV rows for a single advisory. Tolerates shape differences. Includes KEV=1/0 based on CVE match."""
    ...
    # identifiers & cwes are lists of objects
    def _norm_ids(ids, kind): return [i.get("value") for i in (ids or []) if i.get("type") == kind]
    cve_list = _norm_ids(a.get("identifiers"), "CVE")
    ...
    # KEV flag: 1 if ANY CVE for this advisory is in KEV set
    kev_flag = 1 if any(cve in kev_set for cve in cve_list) else 0

    # EPSS: accept list OR object OR missing
    epss_raw = a.get("epss")
    epss_pct = None
    epss_prc = None
...
def write_csv(csv_path: Path, rows: List[Dict[str, Any]]):
    fieldnames = [
        ...
        "KEV", "epss_percentage", "epss_percentile",
...
def main():
    ap = argparse.ArgumentParser(
        description="Download GitHub Advisory Database via REST, zip advisories by severity, and generate a vulnerabilities CSV (with KEV flag)."
    )
    ...
    # KEV
    ap.add_argument("--kev-source", help="CISA KEV source (URL or local file .json/.csv). If omitted, common CISA URLs are tried.")
    ...
    # Load KEV CVE set
    kev_set = load_kev_set(args.kev_source)
    ...
    for advisory in fetch_all_advisories(args.api_base, token, params):
        ...
        all_rows.extend(build_csv_rows_from_advisory(advisory, kev_set))
        total += 1
        if args.sleep > 0:
